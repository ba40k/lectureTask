...
class Library {
    ...
    std::map<uint64_t,std::vector<std::shared_ptr<Book>>> usersBooks;
    public:
    Library() = default;
    int getSize() const;
    ...
    std::vector<User> getUsersListed() const;
    std::vector<Book> getUserBooksListed(const std::shared_ptr<User> &user) ;
